...
上传图片
  ...
  -> 后端选择模型并执行推理
  -> 后端返回标准化结果
  -> 前端展示原图 / 叠加图 / 病害列表 / 量化信息
  -> 用户导出 JSON、图片或回看历史
```

这条链路是项目最重要的主线。只要这条链路稳定，后续不管换模型、加批量、加历史，系统都能继续演进。
...
- `backend/` 负责模型推理、接口协议、结果持久化和历史数据管理
- `frontend/` 负责上传、识别结果展示、筛选对比、导出和历史回看
- 这两层通过统一结果协议连接，不应该互相依赖内部实现细节
...
7. [plan/phase4/phase4.md](./plan/phase4/phase4.md)
