...
def test_external_ultralytics_runner_includes_worker_stderr_on_failure(
    tmp_path: Path,
    monkeypatch,
) -> None:
    weights_path = tmp_path / "model.pt"
    ...
    def fake_run(command, cwd, check, capture_output, text):  # noqa: ANN001
        raise subprocess.CalledProcessError(
            returncode=1,
            cmd=command,
            stderr="segment runtime exploded",
        )
    ...
    try:
        ...
        assert "segment runtime exploded" in str(exc)
    else:
        raise AssertionError("Expected worker stderr to be surfaced")
