...
class DualBranchEnhanceRunner:
    ...
    def enhance(self, img: Image.Image) -> Image.Image:
        ...
        revised_out = self._run_branch_safely(self.revised_G, img_rgb, mode="revised")

        # Step 2: Run Bridge Branch
        bridge_out = self._run_branch_safely(self.bridge_G, img_rgb, mode="bridge")
    ...
    def _run_bridge_core(self, G: BridgeGenerator, img: Image.Image, size: int) -> Image.Image:
        ...
        with torch.amp.autocast(device_type=self.device.type, enabled=self.device.type == "cuda"):
            ...
            residual_smooth = self._gaussian_blur(
                residual, kernel_size=self.bridge_blur_kernel, sigma=self.bridge_blur_sigma
            )

            edge_mask = self._compute_edge_mask(
                x, edge_gain=self.bridge_edge_gain, edge_threshold=self.bridge_edge_threshold
            )
